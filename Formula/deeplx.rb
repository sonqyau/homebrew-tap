class Deeplx < Formula
  desc "Deeplx: OwO-Network/DeepLX"
  homepage "https://github.com/OwO-Network/DeepLX"
  version "1.1.0"
  license "MIT"
  head "https://github.com/OwO-Network/DeepLX.git", branch: "main"
  ...
  def install
    bin.install Dir["deeplx_*"].first.to_s => "deeplx"
    (var / "deeplx").mkpath
    (var / "log").mkpath
  end

  def uninstall
    system "brew", "services", "stop", name.to_s if (var / "log/deeplx.log").exist?
    (var / "log/deeplx.log").unlink if (var / "log/deeplx.log").exist?
    (var / "deeplx").rm if (var / "deeplx").exist?
    super
  end

  def post_uninstall
    rm_r "/opt/homebrew/etc/deeplx" if File.exist?("/opt/homebrew/etc/deeplx")
  end
